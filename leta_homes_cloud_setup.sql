-- =====================================================
-- LETA HOMES AGENCY - Cloud Compatible Database Setup Script
-- Compatible with Cloud Clever, Render, cPanel/phpMyAdmin
-- =====================================================
-- INSTRUCTIONS FOR CLOUD CLEVER:
-- 1. Create MySQL database named 'leta_homes' in Cloud Clever dashboard
-- 2. Create user and grant ALL PRIVILEGES on 'leta_homes'
-- 3. Use phpMyAdmin/External MySQL client/CLI: mysql -h HOST -u USER -p DB < lete_homes_cloud_setup.sql
-- 4. Update php/config.php with Cloud Clever MYSQL_ADDON_* env vars
-- =====================================================
...
-- =====================================================
-- TENANTS TABLE (FIXED: Added missing comma)
-- =====================================================
...
-- =====================================================
-- VIEW: vw_tenant_summary (Arrears/Statements)
-- =====================================================
...
-- =====================================================
-- VIEW: vw_monthly_reports (Monthly Reports)
-- =====================================================
...
-- =====================================================
INSERT INTO `rent_payments` (`tenant_id`, `payment_date`, `amount_paid`, `commission_percentage`, `commission_amount`, `payment_status`, `payment_month`, `payment_year`, `notes`)
SELECT t.id, '2026-02-01', 15000.00, 10.00, 1500.00, 'Paid', '2026-02-01', 2026, 'January Rent'
FROM `tenants` t WHERE t.tenant_id = 'TH-2026-0001'
AND NOT EXISTS (SELECT 1 FROM `rent_payments` WHERE `tenant_id` = t.id AND `payment_date` = '2026-02-01');

INSERT INTO `rent_payments` (`tenant_id`, `payment_date`, `amount_paid`, `commission_percentage`, `commission_amount`, `payment_status`, `payment_month`, `payment_year`, `notes`)
SELECT t.id, '2026-02-15', 25000.00, 10.00, 2500.00, 'Paid', '2026-02-01', 2026, 'February Rent'
FROM `tenants` t WHERE t.tenant_id = 'TH-2026-0002'
AND NOT EXISTS (SELECT 1 FROM `rent_payments` WHERE `tenant_id` = t.id AND `payment_date` = '2026-02-15');

-- =====================================================
-- SAMPLE RECEIPTS
-- =====================================================
INSERT INTO `receipts` (`receipt_number`, `payment_id`, `tenant_id`)
SELECT 'RCP-2026020001', rp.id, rp.tenant_id
FROM `rent_payments` rp JOIN `tenants` t ON rp.tenant_id = t.id 
WHERE t.tenant_id = 'TH-2026-0001' AND rp.payment_date = '2026-02-01'
AND NOT EXISTS (SELECT 1 FROM `receipts` WHERE `receipt_number` = 'RCP-2026020001');

INSERT INTO `receipts` (`receipt_number`, `payment_id`, `tenant_id`)
SELECT 'RCP-2026020002', rp.id, rp.tenant_id
FROM `rent_payments` rp JOIN `tenants` t ON rp.tenant_id = t.id 
WHERE t.tenant_id = 'TH-2026-0002' AND rp.payment_date = '2026-02-15'
AND NOT EXISTS (SELECT 1 FROM `receipts` WHERE `receipt_number` = 'RCP-2026020002');
...
-- VERIFICATION
-- =====================================================
SELECT 'Database setup completed successfully for Cloud Clever!' AS status;
SHOW TABLES;
